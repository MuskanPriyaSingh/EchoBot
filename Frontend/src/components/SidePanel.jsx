...
const SidePanel = () => {
    ...
    // Function to load prompt and set it as recent prompt
    const loadPrompt = async (prompt) => {
        setRecentPrompt(prompt);
        await onSent(prompt);
    };

    // Function to clear localStorage and update the context
    const clearHistory = () => {
        localStorage.removeItem('prevPrompts');  // Remove previous prompts from localStorage
        setPrevPrompts([]);  // Update the context (clear the history in the app)
    };
        ...
        <div className='hidden min-h-[100vh] sm:inline-flex flex-col justify-between bg-gray-800 py-6 px-5'>
            ...
            <div>
                ...
                <div 
                    ...
                    className='mt-5 flex items-center gap-4 p-3 text-[14px] text-gray-400 rounded-4xl cursor-pointer hover:bg-gray-700'
                >
                    <FontAwesomeIcon icon={faHistory} color='slategray' />
                    {extended && <p className='text-red-500 font-bold'>Clear History</p>}
                </div>
            </div>
        </div>
    );
