...
const Main = () => {
  const { onSent, recentPrompt, showResult, loading, resultData, setInput, input, userName } = useContext(Context)
  ...
  const message1 = `Hello, ${userName || 'User'}!`
  ...
  useEffect(() => {
    ...
    if (index === 0) {
      if (text1.length < message1.length) {
      ...
      if (text2.length < message2.length) {
        typingInterval = setInterval(() => {
    ...
    <div className='flex-1 min-h-[100vh] pb-[15vh] bg-gray-900 relative'>
      <div className="flex items-center justify-between p-3 text-gray-300 border-b border-gray-800 bg-gray-900/80 backdrop-blur-md shadow-md">
        ...
        <div className="relative flex items-center gap-3">
          <p className="hidden sm:block text-sm text-gray-400">{greeting}, {userName || "User"}!</p>
          <img
            src={user}
            alt="User Avatar"
          ...
          {menuOpen && (
            <div ref={menuRef} className="absolute right-0 mt-2 w-48 bg-gray-800 text-white rounded-lg shadow-lg p-3">
              <ul className="space-y-2">
                <li className="flex items-center gap-2 cursor-pointer p-2 hover:bg-gray-700 rounded text-red-400">
                  <FaSignOutAlt /> Logout
                </li>
              </ul>
            </div>
      ...
      <div className='max-w-4xl m-auto'>
        {!showResult ? (
          <>
            <div className="my-8 text-4xl sm:text-5xl font-medium p-5">
              ...
              {index === 1 && (
                <p className="text-gray-500 fade-in">{text2}</p>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 p-5 text-gray-300">
              {[
                ...
                { text: "Convert this paragraph into bullet points", icon: faListUl }
              ].map((item, index) => (
                <button
                  key={index}
        ...
        <div className="absolute bottom-0 w-full max-w-4xl px-5 m-auto">
          <div className="flex items-center justify-between gap-3 sm:gap-5 bg-gray-800/90 backdrop-blur-md border border-gray-700 py-3 px-5 rounded-3xl shadow-md shadow-gray-900/30">
            <input
              onChange={(e) => setInput(e.target.value)}
              value={input}
              type="text"
              placeholder="Ask anything..."
              className="flex-1 bg-transparent border-none outline-none p-3 text-sm sm:text-lg md:text-base text-gray-300 placeholder-gray-500 transition-all duration-200 focus:ring-0"
            />
